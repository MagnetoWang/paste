...
package dl4j.modelimport.tensorflow;

import dl4j.utils.download.DownloaderUtility;
...
public class LoadTensorFlowMNISTMLP {
    ...
    protected static Map<String, INDArray> readPlaceholdersAndPredictions() throws IOException {
        ...
        for (int i = 0; i < toReadList.length; i++) {
            String varShapePath = new File(dataLocalPath,toReadList[i] + ".shape").getAbsolutePath();
            String varValuePath = new File(dataLocalPath,toReadList[i] + ".csv").getAbsolutePath();
